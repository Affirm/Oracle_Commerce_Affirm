...
<dsp:getvalueof var="isAffirmOrder" value="false" scope="request"/>
<dsp:getvalueof var="paymentType" bean="ShoppingCart.current.paymentGroups[0].paymentMethod" scope="request"/>



<c:if test="${paymentType == 'affirmPayment'}">
<dsp:getvalueof var="isAffirmOrder" value="true" scope="request"/>
</c:if>
...
<c:choose>
   <c:when test="${isAffirmOrder}">
<%--
   <dsp:form action="${pageContext.request.requestURI}" method="post" name="affirmForm" formid="affirmForm">
        <span class="atg_store_basicButton">
	<dsp:input type="submit" bean="/affirm/commerce/order/purchase/AffirmCheckoutFormHandler.affirmCheckout" 
                     id="atg_store_placeMyOrderButton" value="PAY WITH AFFIRM"
                     iclass="atg_store_actionSubmit"/>
	</span>
   </dsp:form>
--%>
     <!-- <a href="affirm_checkout.jsp">PAY WITH AFFIRM</a> -->
   </c:when>
   <c:otherwise>
