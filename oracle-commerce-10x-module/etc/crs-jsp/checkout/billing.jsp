...
<dsp:page>
  ...
  <dsp:importbean bean="/affirm/commerce/order/purchase/AffirmBillingFormHandler" />
<dsp:importbean var="affirmConfig" bean="/affirm/commerce/payment/AffirmPaymentConfiguration" />

  <c:set var="stage" value="billing"/>

<p>OrderId: <dsp:valueof bean="ShoppingCart.current.id"/>
<dsp:getvalueof var="showAffirmPaymentOption" value="${affirmConfig.enabled}" scope="request"/>  

<c:if test="${showAffirmPaymentOption}">
<dsp:droplet name="/affirm/rules/AffirmCheckoutRuleDroplet">
  <dsp:param name="order" bean="ShoppingCart.current"/>
  <dsp:param name="pageType" value="cart"/>
  <dsp:oparam name="output">
    <dsp:getvalueof var="dataPromoId" param="dataPromoId" scope="request"/>
    <dsp:getvalueof var="dataAmount" param="dataAmount" scope="request"/>
    <!-- Affirm data prom id: ${dataPromoId} -->
    <!-- Affirm data amount: ${dataAmount} -->
  </dsp:oparam>
</dsp:droplet>
<dsp:getvalueof var="shipGroupCountry" bean="ShoppingCart.current.shippingGroups[0].shippingAddress.country"/>
<br>shipGroupCountry: ${shipGroupCountry}
<c:if test="${shipGroupCountry != 'USA' && shipGroupCountry != 'US'}">
	<dsp:getvalueof var="showAffirmPaymentOption" value="false" scope="request"/>
</c:if>

<br/>${showAffirmPaymentOption}

</c:if>

</p>
<script>
	function paymentTypeClicked(obj)
	{
		//alert(obj.id);	
		if(obj.id == 'creditcard')
		{
			document.getElementById('affirmSection').style.display = 'none';
			document.getElementById('affirmPayWithSavedAddress').style.display = 'none';



			
			document.getElementById('creditCardSection').style.display = '';
			document.getElementById('creditCardNewAddress').style.display = '';
			document.getElementById('creditCardPayWithSavedAddress').style.display = '';
		
		}
		else if(obj.id == 'affirm'){
			
			document.getElementById('creditCardSection').style.display = 'none';
			document.getElementById('creditCardNewAddress').style.display = 'none';			
			document.getElementById('creditCardPayWithSavedAddress').style.display = 'none';
			
			document.getElementById('affirmSection').style.display = '';
			document.getElementById('affirmPayWithSavedAddress').style.display = '';
        ...
        <crs:checkoutContainer currentStage="${stage}" title="${title}">
          ...
          <jsp:attribute name="formErrorsRenderer">
            ...
            <dsp:include page="gadgets/checkoutErrorMessages.jsp">
              <dsp:param name="formHandler" bean="AffirmBillingFormHandler"/>
            </dsp:include>
